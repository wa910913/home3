...
source("themes/variant/utils/r/hugoup.r",environment(),encoding="UTF-8")
  ...
  #knitr::opts_knit$set(root.dir = dirname(infile)) #註解: v1
  #knitr::opts_knit$set(base.dir = dirname(infile)) #註解: v1
  knitr::opts_chunk$set(
    #fig.path = glue::glue("{base_name}_files/figure-html/")
    fig.path = glue::glue("{knitr_files_root}/figure-html/")
  )
  # 無效cleanold(file.path(dirname(infile),knitr::opts_chunk$get("fig.path")))
  ...
  setwd(dirname(infile)) # 註解:v2, 這一行抵銷掉root.dir,base.dir
  ...
  if (length(deps)) {
    # html_dependencies_as_string()第2參數為子目錄,第3個為根目錄?
    # m = rmarkdown:::html_dependencies_as_string(deps, file.path(dirname(infile),knitr_files_root), dirname(infile)) # 註解v1:
    m = rmarkdown:::html_dependencies_as_string(deps, knitr_files_root,".") # 註解v2:不僅會複製檔案,而且會傳會放在md的連結
    if (length(i <- grep("^---\\s*$", md)) >= 2) { # 只是把上面m內容放到md檔案結尾
      md = append(md, m, i[2])
    }
